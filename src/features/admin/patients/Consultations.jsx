...
export default function Consultations({ patientId }) {
    ...
    useEffect(() => {
        const fetchConsultations = async () => {
            ...
            setError(null)
            try {
                const response = await getPatientConsultationsService(patientId)
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Dernières consultations du patient.</p>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-200 bg-white">
                            {consultations.map((consultation) => (
                                <tr key={consultation.id} className="transition hover:bg-slate-50">
                                    <td className="px-4 py-4">{consultation.medecin_name || consultation.medecin?.user?.name || '—'}</td>
                                    <td className="px-4 py-4">{consultation.diagnostic || '—'}</td>
                                    <td className="px-4 py-4">{consultation.traitement || '—'}</td>
                                    <td className="px-4 py-4">{consultation.details?.temperature || '—'}</td>
                                    <td className="px-4 py-4">{consultation.details?.tension || '—'}</td>
