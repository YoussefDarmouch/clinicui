...
const summaryCards = [
    ...
    { label: 'M�decins', valueKey: 'total_medecins', accent: 'from-primary-500 to-primary-600' },
        ...
        <div className="space-y-6">
            <div className="rounded-[2rem] border border-slate-200 bg-slate-950/90 p-6 shadow-[0_24px_80px_-32px_rgba(15,23,42,0.8)] text-white backdrop-blur-xl">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <p className="text-sm uppercase tracking-[0.28em] text-primary-100">Vue d�ensemble</p>
                        <h1 className="mt-3 text-3xl font-semibold">Tableau de bord clinique</h1>
                        <p className="mt-2 max-w-2xl text-sm text-slate-300">
                            Surveillez les indicateurs cl�s de la clinique et suivez l�activit� utilisateur en un coup d��il.
                        </p>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-5">
                {summaryCards.map((card) => (
                    ...
                    >
                        ...
                        <p className="mt-3 text-sm text-slate-100/80">Comparaison du mois pr�c�dent</p>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.3fr_0.7fr]">
                <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-2 text-sm text-slate-500">�volution des consultations r�centes.</p>
                        </div>
                    ...
                    <div className="mt-6">
                        <div className="flex h-52 items-end gap-3">
                            {monthlyData.length > 0 ? (
                                ...
                                <div className="flex h-52 items-center justify-center text-sm text-slate-400">Aucune donn�e de consultation disponible.</div>
                            )}
                        </div>
                        <div className="mt-4 grid grid-cols-6 gap-2 text-center text-xs uppercase tracking-[0.2em] text-slate-400">
                            {monthlyData.length > 0 ? (
                                ...
                                <span className="col-span-6">Aucune p�riode disponible</span>
                            )}
                        </div>
                    </div>
                </section>
                ...
                <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h2 className="text-2xl font-semibold text-slate-900">R�partition des consultations</h2>
                            <p className="mt-2 text-sm text-slate-500">Top sp�cialit�s actives.</p>
                        </div>
                    ...
                    <div className="mt-8 flex flex-col items-center gap-6 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="space-y-3">
                            {specialityData.length > 0 ? (
                                ...
                                <div className="text-sm text-slate-500">Aucune sp�cialit� disponible pour le moment.</div>
                            )}
                        </div>
                    </div>
                </section>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.35fr_0.65fr]">
                <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h2 className="text-2xl font-semibold text-slate-900">Consultations r�centes</h2>
                            <p className="mt-2 text-sm text-slate-500">Derni�res consultations enregistr�es.</p>
                        </div>
                        ...
                        <table className="min-w-full text-left text-sm text-slate-600">
                            <thead className="border-b border-slate-200 text-slate-500">
                                <tr>
                                    ...
                                    <th className="py-3 font-medium">M�decin</th>
                                    <th className="py-3 font-medium">Sp�cialit�</th>
                                ...
                                {recentConsultationRows.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan="5" className="py-10 text-center text-sm text-slate-500">
                                            Aucune consultation r�cente disponible.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <div>
                        <h2 className="text-2xl font-semibold text-slate-900">Activit�s r�centes</h2>
                        <p className="mt-2 text-sm text-slate-500">Derni�res actions du tableau de bord.</p>
                    </div>

                    <div className="mt-6 space-y-4">
                        {activityRows.length > 0 ? (
                            activityRows.map((item, idx) => {
                                const title = item?.label ?? item?.title ?? item?.action ?? item?.type ?? 'Action r�cente';
                                ...
                                const isPositive = /ajout|cr��|nouveau|nouvelle|new/i.test((title || '') + ' ' + (description || ''));
